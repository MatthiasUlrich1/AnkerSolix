/**
 * Register maps derived from Anker official HA integration (MIT):
 * https://github.com/anker-charging/ha-anker-solix-official
 * Stage 2: sensors plus holding-register writes (mode, SOC, setpoint, plug).
 */

import type { ModbusControlSpec, ModbusDeviceProfile, ModbusProfileId, ModbusReadQuantity } from "./types";
import { OPERATING_MODE_CAPABILITY_BITS, OPERATING_MODE_LABELS, OPERATING_MODE_OPTIONS } from "./types";

const SOLARBANK_RANGES = [
	{ type: "input" as const, start: 10000, end: 10050 },
	{ type: "input" as const, start: 10090, end: 10156 },
	{ type: "input" as const, start: 10208, end: 10265 },
	{ type: "input" as const, start: 32768, end: 32774 },
	{ type: "holding" as const, start: 10060, end: 10072 },
	{ type: "holding" as const, start: 10074, end: 10081 },
	{ type: "holding" as const, start: 60000, end: 60003 },
];

const SOLARBANK_QUANTITIES: Record<string, ModbusReadQuantity> = {
	device_model: {
		address: 32768,
		dataType: "STRING",
		count: 5,
...
const SOLARBANK_CONTROLS: ModbusControlSpec[] = [
	{
		id: "operating_mode",
		name: "Operating mode",
		kind: "select",
		...
		optionLabels: OPERATING_MODE_LABELS,
		capabilityBits: OPERATING_MODE_CAPABILITY_BITS,
	},
	{
...
const SMART_METER_GEN2: ModbusDeviceProfile = {
	id: "smartMeterGen2",
...
const SMART_PLUG_GEN2: ModbusDeviceProfile = {
	id: "smartPlugGen2",
	...
	ranges: [
		{ type: "holding", start: 30000, end: 30037 },
	...
	quantities: {
		...
		temperature: { address: 30037, dataType: "INT16", count: 1, gain: 10, unit: "°C", name: "Temperature" },
		switch_status: {
			address: 30029,
			dataType: "UINT16",
			count: 1,
...
export const MODBUS_PROFILES: Record<ModbusProfileId, ModbusDeviceProfile> = {
...
export const MODBUS_PROFILE_ORDER: ModbusProfileId[] = [
	"solarbank4",
	"solarbankMaxAc",
	"solarbankMax",
	"smartMeterGen2",
...
export function getModbusProfile(id: ModbusProfileId): ModbusDeviceProfile {
	return MODBUS_PROFILES[id];
}

export function matchProfileByProductCode(code: string): ModbusDeviceProfile | undefined {
