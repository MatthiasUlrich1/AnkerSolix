/**
 * HTML dashboard widgets inspired by ioBroker.solix4 (Michael Horn / michihorn64).
 *
 * @see https://github.com/michihorn64/ioBroker.solix4
 * Layout and UX concept used with permission; reimplemented for anker-solix state tree (MIT).
 */
...
export interface DashboardSiteSnapshot {
	...
	solarbankName: string;
	solarbankModel: string;
	solarbankOnline: boolean | null;
	smartmeterOnline: boolean | null;
	devices: DashboardDeviceRow[];
	settings: {
		appOutputPower: number | null;
		homeLoadPreset: number | null;
		chargeUpperLimit: number | null;
		dischargeLowerLimit: number | null;
		allowGridExport: boolean | null;
		acInputLimit: number | null;
...
function evaluateWarnings(site: DashboardSiteSnapshot): {
	healthClass: string;
	healthText: string;
	warnings: string[];
} {
	...
	if (offline > 0) {
		warnings.push(`${offline} Gerät(e) offline`);
	}
	if (site.soc <= 15) {
	...
	if (site.batteryTemp !== null && site.batteryTemp >= 50) {
		...
		warnings.push("Batterietemperatur erhöht");
	}
	const healthClass = warnings.some(
		...
		: warnings.length
			? "warn"
			: "good";
	return { healthClass, healthText: warnings.length ? warnings.join(" · ") : "Alles OK", warnings };
}
...
export function buildLiveWidgetHtml(site: DashboardSiteSnapshot): string {
	const charge = site.batteryCharge;
	const discharge = site.batteryDischarge;
	const batteryMode = charge > 0 ? "Lädt" : discharge > 0 ? "Entlädt" : "Bereit";
...
<div class="sl-head"><div class="sl-title-wrap"><div class="sl-logo">☀️</div><div><div class="sl-title">${escapeHtml(site.siteName)}</div><div class="sl-subtitle">${escapeHtml(site.solarbankName)} · ${escapeHtml(site.solarbankModel)}</div></div></div><div class="sl-health ${healthClass}">${escapeHtml(healthText)}</div></div>
<div class="sl-flow">
...
<div class="sl-card home"><div class="sl-icon">🏠</div><div class="sl-name">HAUSVERBRAUCH</div><div class="sl-value">${watts(site.home)}</div><div class="sl-secondary">Jetzt</div></div>
...
<div><div class="sl-battery-name">AKKU · ${batteryMode.toUpperCase()}</div><div class="sl-battery-value">${soc.toFixed(0)} % · ${watts(batteryPower)}</div><div class="sl-secondary">${temp(site.batteryTemp)}</div><div class="sl-bar"><div class="sl-bar-fill" style="width:${soc}%;background:${soc >= 60 ? "#4bd14e" : soc >= 25 ? "#ffbb33" : "#ff5b5b"}"></div></div></div>
</div></div>
<div class="sl-status">
...
<div class="sl-footer"><span>Geräte: ${onlineDevices}/${site.devices.length} online${offlineDevices ? ` · ${offlineDevices} offline` : ""}</span><span>Site: ${escapeHtml(site.siteKey)}</span><span>Aktualisiert: ${escapeHtml(updated)}</span></div>
...
export function buildSettingsWidgetHtml(site: DashboardSiteSnapshot): string {
	const s = site.settings;
	const tile = (icon: string, title: string[], value: string, note: string, cls: string): string =>
		`<div class="ss-tile ${cls}"><div class="ss-tile-icon">${icon}</div><div class="ss-tile-title">${title.map(l => `<span>${escapeHtml(l)}</span>`).join("")}</div><div class="ss-tile-value">${escapeHtml(value)}</div><div class="ss-tile-note">${escapeHtml(note)}</div></div>`;
...
${tile("🏠", ["HAUSLAST-", "ZIELWERT"], s.homeLoadPreset !== null ? watts(s.homeLoadPreset) : "–", "set_output_power", "orange")}
</div></div>
<div class="ss-group green"><div class="ss-group-title">🔋 AKKU</div><div class="ss-grid two">
${tile("🔋", ["LADEGRENZE", "OBEN"], percent(s.chargeUpperLimit), "max_charge_soc", "green")}
${tile("🔋", ["ENTLADEGRENZE", "UNTEN"], percent(s.dischargeLowerLimit), "min_soc", "green")}
</div></div>
<div class="ss-group blue"><div class="ss-group-title">🌐 NETZ</div><div class="ss-grid three">
${tile("↻", ["NETZEIN-", "SPEISUNG"], exportText, "allow_grid_export", "blue")}
${tile("🔌", ["AC-EINGANGS-", "GRENZE"], s.acInputLimit !== null ? watts(s.acInputLimit) : "–", "ac_input_power", "blue")}
${tile("⚙", ["BETRIEBS-", "MODUS"], s.operatingMode || "–", "preset_usage_mode", "blue")}
</div></div>
</div></div>`;
}

export function buildDashboardWidgetHtml(site: DashboardSiteSnapshot): string {
	const live = buildLiveWidgetHtml(site);
	const settings = buildSettingsWidgetHtml(site);
	const strip = (html: string): string => html.replace(/<style>[\s\S]*?<\/style>/g, "");
...
export function buildEnergyWidgetHtml(site: DashboardSiteSnapshot): string {
	const e = site.energy;
	const tile = (icon: string, label: string, value: string, cls: string): string =>
		`<div class="se-tile ${cls}"><div class="se-icon">${icon}</div><div class="se-label">${escapeHtml(label)}</div><div class="se-value">${escapeHtml(value)}</div></div>`;
	return `
<style>
...
<div class="se-head"><div><div class="se-title">Energie heute · ${escapeHtml(site.siteName)}</div><div class="se-subtitle">${escapeHtml(site.siteId)}</div></div></div>
<div class="se-grid">
${tile("☀️", "Solarproduktion", kwh(e.solar), "solar")}
${tile("🏠", "Hausverbrauch", kwh(e.home), "home")}
${tile("⚡", "Netzbezug", kwh(e.gridImport), "grid")}
${tile("↗", "Einspeisung", kwh(e.gridExport), "export")}
${tile("🔋", "Akku geladen", kwh(e.batteryCharge), "battery")}
${tile("🔋", "Akku entladen", kwh(e.batteryDischarge), "battery")}
${tile("🛡️", "Autarkie", e.autarky !== null ? `${e.autarky.toFixed(1)} %` : "–", "metric")}
${tile("♻️", "Eigenverbrauch", e.selfConsumption !== null ? `${e.selfConsumption.toFixed(1)} %` : "–", "metric")}
</div></div>`;
}

export function buildDiagnosisWidgetHtml(site: DashboardSiteSnapshot): string {
	...
	const warningCards =
		warnings.length > 0
			? warnings
					.map(
						w =>
							`<div class="sd-warning ${w.includes("kritisch") || w.includes("offline") || w.includes("getrennt") ? "error" : "warning"}"><div class="sd-warning-title">${escapeHtml(w)}</div></div>`,
					)
					.join("")
			: `<div class="sd-warning ok"><div class="sd-warning-title">🟢 Keine Warnungen</div></div>`;
	const deviceRows = site.devices
		.map(
			d =>
				`<tr><td><span class="sd-dot ${dot(d.online)}"></span></td><td>${escapeHtml(d.typeLabel)}</td><td>${escapeHtml(d.name)}</td><td>${escapeHtml(d.model)}</td></tr>`,
		)
		.join("");
	return `
<style>
...
<div class="sd-card"><div class="sd-label">Geräte</div><div class="sd-value">${site.devices.length}</div></div>
...
export function buildDeviceInventoryHtml(site: DashboardSiteSnapshot): string {
	const typeSummary = new Map<string, { label: string; count: number; online: number; offline: number }>();
	...
	const typeCards = [...typeSummary.values()]
		.map(
			t =>
				`<div class="sdi-type"><div class="sdi-type-title">${escapeHtml(t.label)}</div><div class="sdi-type-count">${t.count}</div><div class="sdi-type-meta">${t.online} online · ${t.offline} offline</div></div>`,
		)
		.join("");
	const rows = site.devices
		.map(
			d =>
				`<tr><td><span class="sdi-dot ${dot(d.online)}"></span></td><td>${escapeHtml(d.typeLabel)}</td><td>${escapeHtml(d.name)}</td><td>${escapeHtml(d.model)}</td><td>${escapeHtml(d.key)}</td></tr>`,
		)
		.join("");
	return `
<style>
...
<div class="sdi-title">Geräteübersicht ${escapeHtml(site.siteName)}</div>
<div class="sdi-types">${typeCards || "Keine Geräte"}</div>
<table><thead><tr><th>Status</th><th>Typ</th><th>Name</th><th>Modell</th><th>ID</th></tr></thead><tbody>${rows}</tbody></table>
</div>`;
}

export function buildOverviewHtml(sites: DashboardSiteSnapshot[]): string {
	const rows = sites
		.map(s => {
...
<div class="so-title">Anlagen-Übersicht (${sites.length})</div>
...
export function numEntity(entities: Record<string, unknown>, key: string): number {
	return n(entities[key]);
}

export function optionalNum(entities: Record<string, unknown>, key: string): number | null {
...
export function optionalBool(entities: Record<string, unknown>, key: string): boolean | null {
	const v = entities[key];
...
export function cloudOnline(entities: Record<string, unknown>): boolean | null {
	const raw = entities.cloud_state ?? entities.mqtt_connection;
	if (raw === undefined || raw === null || raw === "") {
		return null;
	}
	const text =
		typeof raw === "string" || typeof raw === "number" || typeof raw === "boolean" ? String(raw).toLowerCase() : "";
